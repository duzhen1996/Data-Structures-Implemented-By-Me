...
所以我们想一个问题，如果删了97或者72怎么办，这就需要重构非叶节点索引，如果一个节点的最大key被删除或者因为key的插入导致最大key被其他的key覆盖，这种我们都要进行向上的索引递归重构。
...
这就只能合并，合并的方法很简单，左右随便找一个合并就好了，首先，夹在两个节点之间的父节点key会消失，然后两个节点合并并且向上递归重构父索引。
