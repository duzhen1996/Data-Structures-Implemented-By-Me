...
B树的设计很简单。B树有一个概念，叫做阶。一个阶为m的节点非叶非根的节点一红有不多于m个不少于2/m个子树。B树的节点除了有一串n个指向子树的指针之外，还有n-1个key值来索引。低a个子树的所有key值都在a-1和a号键值之间。其实这就是一个查找树，只是节点元素不固定的多叉查找树而已。
...
这里的几张图展现了“向兄弟要”和“与父节点的元素合并”两个过程，向兄弟要我们可以向做兄弟要，也可以向右兄弟要。当左右兄弟有要不起的时候，我们只能从左右挑一个兄弟和自己合并。
...
然后我们就可以说删除了，删除也分两种，一种是叶节点，我们直接删就好（因为已经做过处理）。然后就是如果是非叶节点怎么办，这里就使用后继key或者前趋key放到这个要删除的非叶节点的key上，准确来说，是先去删除叶节点的key，然后再把那个key替换到要删除的key的位置上。当然，这个删除也是有讲究的，其实就是提前考虑了节点的提前扩容问题，首先就是我们看看我们要找前趋还是后继，这个和当前key左右子树的是不是有至少m/2节点有关系，如果左边的key个数超过了，那就删除前趋，如果右边那就后继。

如果两边都只有m/2-1那就进行一次key下降的合并操作，然后在合并之后的节点中递归调用key的删除就好了。
